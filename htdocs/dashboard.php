<?php
// Establish connection to the inventory database and fetch data
require_once 'config.php'; // Ensure this file contains INV_DB_HOST, INV_DB_USER, INV_DB_PASS, INV_DB_NAME

// Create a new connection for the inventory database
$conn = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);

if ($conn->connect_error) {
    // If connection fails, we'll handle it gracefully in JS with an empty array
    $inventory_data = [];
} else {
    $inv_sql = "SELECT sku, product_name, stock, min_threshold, last_update FROM inventory ORDER BY sku ASC";
    $inv_result = $conn->query($inv_sql);
    $inventory_data = [];
    if ($inv_result && $inv_result->num_rows > 0) {
        while ($row = $inv_result->fetch_assoc()) {
            // Match the keys to what the JavaScript expects
            $inventory_data[] = [
                'id' => $row['sku'],
...
<body class="bg-gray-100 font-sans antialiased text-gray-800">
        ...
        <div class="container mx-auto bg-white rounded-xl shadow-lg p-6 md:p-8">
            ...
            <main id="dashboard-content" class="hidden grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-gray-50 p-6 rounded-lg shadow-md transform transition-transform duration-300 hover:scale-[1.01]">
                    ...
                    <p class="mt-4 text-sm text-gray-600">Inventory updates in real-time from the warehouse database.</p>
                </div>
                ...
                <h2 class="mb-4 text-2xl font-semibold text-gray-800" style="text-align: center;">📋 Warehouse RFID Logs</h2>
                    <p class="text-center text-sm text-gray-500 mb-4">Click on column headers like 'ID' or 'Date' to sort the data.</p>
                    ...
                    <div class="table-responsive">
                        <table id="rfidTable" class="table table-striped table-bordered w-full">
                            ...
                            <tbody id="rfidBody">
                                <?php
                                // This part remains the same, connecting to the RFID database
                                $conn = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
                                if ($conn->connect_error) {
                                    die("DB connection failed");
                                }
                                $sql = "SELECT * FROM rfid_logs ORDER BY id DESC"; // Already fetching in descending order
    ...
    <script>
        // Make the inventory data from PHP available to JavaScript
        const dbInventoryData = <?php echo json_encode($inventory_data); ?>;
        ...
        document.addEventListener('DOMContentLoaded', () => {
             ...
             * Now uses data fetched from the database via PHP.
             */
            async function fetchData() {
                ...
                // Use inventory data fetched from the database
                inventory = dbInventoryData;

                loader.style.display = 'none';
                dashboardContent.style.display = 'grid';

                // Render all components with the live data
                renderInventoryTable();
                updateAlerts();
                startRobotSimulation();

                // Initialize RFID Table and Charts
                initializeRfidLogs(); 
            }
            ...
            function renderInventoryTable() {
                ...
                if (inventory.length === 0) {
                    ...
                    row.innerHTML = `<td colspan="5" class="text-center py-4 px-4 text-gray-500">No inventory data found in the database.</td>`;
        ...
        function initializeRfidLogs() {
            rfidTable = $('#rfidTable').DataTable({
                ...
                order: [[0, 'desc']], // NEW: Set default sort order to the first column (ID) descending
            ...
            function generateChartData(data, labelType) {
                ...
                const datasets = [];
                const colors = ['#0d6efd','#198754','#ffc107','#dc3545','#6610f2','#20c997'];
